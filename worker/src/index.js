...
【問題番号の読み取りルール（最重要）】
問題番号は必ず【解説ページ OCRテキスト】からのみ取得する。解説ページでは各問の冒頭が「○ 数字」や「× 数字」の形式になっており、○または×の記号の直後に3桁の問題番号が続く。この3桁の整数をそのまま id および source_no に使うこと。問題番号が読み取れない場合は必ず id:0 とすること（プロンプト中の例示数字・前後の問番号・推測値などで補完することは絶対禁止）。
