...
問題番号は必ず【解説ページ OCRテキスト】の各問冒頭にある「○ 数字」または「× 数字」（○/×記号の直後に続く3桁の整数）からのみ取得する。この3桁の整数をそのまま id および source_no に使うこと。
- 【問題ページ OCRテキスト】に現れるいかなる数字も、問題番号として使ってはならない。問題ページには試験年度コード（H28.5-C等。OCRが「428」「525」のように誤読することがある）や設問内の数値が大量に含まれるが、これらは絶対に id にしない。
- 解説ページで○/×直後の3桁整数が読み取れない問題は、必ず id:0 とすること。プロンプト中の例示数字・前後の問番号・問題ページの数字・連番・推測値での補完は一切禁止。
...
async function handleExtract(request, env) {
  ...
  const stripExamCodes = (text) => text
    ...
    .replace(/\d{2,4}[pPｐＰ]/g, ''); // 「321p」「327p」等の参照ページ番号を除去（必修基本書行と分離して抽出された場合も対処）
