...
【問題番号の読み取りルール（最重要・絶対厳守）】
問題番号は必ず【解説ページ OCRテキスト】の各問の行頭から取得する。解説ページの各問は次のレイアウトで始まる：
「正誤マーク（○または×）」→「縦の点線の区切り線」→「3桁の問題番号」→（解説本文）
この区切り線はOCRで「i」「|」「I」「！」「：」などのゴミ文字や空白に化ける。また3桁の問題番号もOCRで桁ごとに空白が入り「4 1 9」「4 2 0」のように分断されて記録されることが多い。これらの空白を詰めて1つの3桁整数（例：「x i 4 1 9」→ 419）として読み取り、id および source_no に使うこと。
- 【問題ページ OCRテキスト】に現れるいかなる数字も、問題番号として使ってはならない。問題ページには試験年度コード（H28.7-ウ等。OCRが「428」のように誤読することがある）や設問内の数値が大量に含まれるが、これらは絶対に id にしない。
- 解説本文の右端には参照ページ（「必修基本書 社会保険科目……321p」など。OCRでは「32 1 P」のように分断される）があるが、この数字（321等）は問題番号ではない。行頭の正誤マーク直後の数字だけが問題番号である。
- 行頭の正誤マーク直後の3桁整数が読み取れない問題は、必ず id:0 とすること。プロンプト中の例示数字・前後の問番号・問題ページの数字・参照ページ番号・連番・推測値での補完は一切禁止。
- 各問の id は解説ページから読んだ実際の値であり、連続している保証はない。前問の番号から機械的に+1してはならない。
...
以下の数字はすべて問題番号ではない。いかなる場合も問題番号として使用しないこと：
1. 過去問出題履歴コード：「H28.7-ウ」「R3.6-B」「H元.5-ア」のような「元号＋年数＋枝番号」形式。OCRが「H」を「4」、「R」を「5」と誤読した「428.7-ウ」等の形でも現れる。
2. ページ右端の章インデックスタブ：「問419〜438」「419〜438」のように章全体の問題範囲を示す数字。この節が419番から始まる場合、全ページのタブに「419」が印字されるが、これは問題番号ではない。
3. ページ番号：ページ下部の「180」「181」等。
4. 参照ページ：各問の解説ヘッダー右端に「必修基本書 社会保険科目……321p」のように印刷されている参照ページ番号。「321p」「327p」「331p」など「数字＋p/P」の形式で現れ、○/×マークの右側（問題番号）ではなく行の右端にある。この数字（321・327・331等）は絶対に問題番号ではない。
5. 法令番号・条文番号：「法78条」「令3条」等に含まれる数字。
...
async function handleExtract(request, env) {
  ...
  // 試験年度コード（H28.5-C / R6.2-A 等）を除去。OCRがH→4, R→5と誤読し、
  // さらに区切り文字（.・スペース）を崩して読むと「428」が問題番号として残るため、
  // 区切り・接尾辞のバリエーションを広く許容して確実に除去する。
  const examCode = /[HR元45][\s]?[\d元]{1,2}[\s.．,，・]{0,3}\d{1,2}[\s]?[-－ー―‐][\s]?[アイウエオＡ-ＺA-Z]/g;
  const stripExamCodes = (text) => text
    .replace(examCode, '')
    .replace(/必修基本書[^\n]*/g, '') // 「必修基本書 社会保険科目……321p」行を除去
    .replace(/\d[\d 　]{0,5}[pPｐＰ]/g, ''); // 参照ページ番号を除去。OCRが「321P」を「32 1  P」のように桁間スペースで分断しても確実に除去する
